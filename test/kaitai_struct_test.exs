defmodule KaitaiStructTest do
  ...
  test "bytes_strip_right/2 correctly strips right-hand-side of binary" do
    assert <<0xFF, 0xAA, 0xBB>> =
             KaitaiStruct.bytes_strip_right(<<0xFF, 0xAA, 0xBB, 0xCC, 0xCC, 0xCC>>, 0xCC)

    assert <<0xFF, 0xCC, 0xAA, 0xBB>> =
             KaitaiStruct.bytes_strip_right(<<0xFF, 0xCC, 0xAA, 0xBB, 0xCC, 0xCC, 0xCC>>, 0xCC)

    assert <<0xFF, 0xCC, 0xAA, 0xBB, 0xCC, 0xCC, 0xCC, 0xAA>> =
             KaitaiStruct.bytes_strip_right(
               <<0xFF, 0xCC, 0xAA, 0xBB, 0xCC, 0xCC, 0xCC, 0xAA>>,
               0xCC
             )
  ...
  test "bytes_terminate/3 correctly terminates string" do
    assert <<0xFF, 0xAA, 0xBB>> =
             KaitaiStruct.bytes_terminate(<<0xFF, 0xAA, 0xBB, 0xCC, 0xCC, 0xCC>>, 0xCC, false)

    assert <<0xFF, 0xAA, 0xBB, 0xCC>> =
             KaitaiStruct.bytes_terminate(<<0xFF, 0xAA, 0xBB, 0xCC, 0xCC, 0xCC>>, 0xCC, true)

    assert <<0xFF, 0xCC, 0xAA, 0xBB, 0xCC, 0xCC, 0xCC>> =
             KaitaiStruct.bytes_terminate(
               <<0xFF, 0xCC, 0xAA, 0xBB, 0xCC, 0xCC, 0xCC>>,
               0xDD,
               false
             )

    assert <<0xFF, 0xCC, 0xAA>> =
             KaitaiStruct.bytes_terminate(
               <<0xFF, 0xCC, 0xAA, 0xBB, 0xCC, 0xCC, 0xCC, 0xAA>>,
               0xAA,
               true
