defmodule KaitaiStruct.Stream do
  ...
  @spec read_bytes_array(stream :: pid(), n :: non_neg_integer()) :: {:ok, [integer()]} | {:error, read_error()}
  ...
  """
  @spec read_bytes_term(stream :: pid(), encoding :: String.t(), term :: integer(), include_term :: boolean(), consume_term :: boolean(), eos_error :: boolean()) :: {:ok, [integer()]} | {:error, read_error()} | {:error, :unsupported_encoding} | {:error, {:term_encoding_error, term()}}
    ...
    if encoding = Map.get(KaitaiStruct.encodings(), encoding) do
      case :unicode.characters_to_binary(term, KaitaiStruct.native_encoding(), encoding) do
        term_bin when is_binary(term_bin) ->
          GenServer.call(pid, {:read, {:bytes_term, term_bin, include_term, consume_term, eos_error}})
  ...
  @impl true
  def handle_call({:read, {:bytes_term, term_bin, include_term, consume_term, eos_error}}, _from, state) do
    ...
    |> Enum.reduce_while({state, <<>>}, fn
      {:ok, {next_byte, state}}, {_, buffer_bin} ->
        ...
        case new_buffer_bin do
          <<without_term::binary-size(byte_size(new_buffer_bin)-byte_size(term_bin)), ^term_bin::binary>> ->
  ...
  def handle_call({:ensure, {:fixed_contents, contents}}, _from, state) do
    ...
    with {:ok, {data, state}} <- stream_bitstring(state, num_bytes) do
      if data == contents do
        ...
        {:reply, {:error, :fixed_contents_mismatch}, state}
      end
    else
      {:error, :reached_eof} ->
  ...
  @spec read_and_reply(stream_state(), num_bytes :: non_neg_integer(), reply_fn :: fun()) :: {:reply, term(), stream_state()}
  defp read_and_reply(state, n, reply_fn) do
    with {:ok, {data, state}} <- stream_bitstring(state, n) do
      {:reply, reply_fn.(data), state}
    else
      error -> {:reply, error, state}
    end
  end

  @spec stream_bit_array(stream_state(), bits :: non_neg_integer()) :: {:ok, {data :: [boolean()], new_state :: stream_state()}} | {:error, read_error()}
  ...
  @spec stream_bitstring(stream_state(), bytes :: non_neg_integer(), opts :: keyword()) :: {:ok, {data :: bitstring(), new_state :: stream_state()}} | {:error, read_error()}
  defp stream_bitstring(state, n_bytes, opts \\ []) do
    ...
    cond do
      ...
      true ->
        ...
        raw_stream_bin = Stream.take(stream, bytes_from_stream) |> Enum.to_list() |> :binary.list_to_bin()

        <<buffer_bin::size(bits_from_buffer), remaining_buffer::size(remaining_buffer_bits)>> = buffer
        <<stream_bin::size(bits_from_stream), remaining_stream::size(remaining_stream_bits)>> = raw_stream_bin

        all_data = if bits_from_buffer > 0, do: <<buffer_bin::size(bits_from_buffer), stream_bin::size(bits_from_stream)>>, else: <<stream_bin::size(bits_from_stream)>>

        new_buffer = if remaining_stream_bits > 0 do
        ...
        {:ok, {all_data, stream_state(state,
